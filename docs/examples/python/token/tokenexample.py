...
def getToken(application_id, secret, host):
    print('Getting access token')
    r = requests.post('{}/v1/oauth/access_token?grant_type=client_credentials&client_id={}&client_secret={}'.format(host, application_id,secret))
...
def main(application_id, secret, host):
    access_token, username = getToken(application_id, secret, host)

    print('Getting the user object using the token in the query parameters')
    r = requests.get('{}/api/users/{}?access_token={}'.format(host, username, access_token))
    print('{} - {}'.format(r.status_code,r.text))

    print('Getting the user object using the token in the Authorization header')
    r = requests.get('{}/api/users/{}'.format(host, username), headers={'Authorization':'token {}'.format(access_token)})
    print('{} - {}'.format(r.status_code,r.text))
...
if __name__ == "__main__":
    ...
    main(args.applicationid, args.secret, args.environment)
